...
- 修复点击任务栏程序后额度组件可能被 Explorer 调整到后层、消失且无法自动恢复的问题。
- 增加低频 Z 顺序保活：默认每 250ms 以单次轻量 Win32 调用恢复任务栏层级，不重新引入 60Hz 空闲循环。

### 0.13.0

- 修正 Codex Radar 的参考价格与参考时间：优先读取官方接口的单任务平均成本与平均耗时，Sol Max 当前正确显示约 `$9.4 / 37分钟`。
- 重构任务栏定位循环：空闲时停止重复移动和显示窗口，仅在位置实际变化时更新，移动动画期间自动临时提升到 60Hz。
- 新增“节能 / 平衡 / 流畅”三档任务栏轮询率，默认采用 250ms 的平衡档，在响应速度和 CPU 占用之间取得平衡。

### 0.12.0

- Release 更新日志改为从 README 自动生成规范中文内容，并修正 v0.11.0 已发布版本的说明。
- 主题切换增加过渡互斥，防止频繁切换造成暗色到亮色时详情快照消失。
- 设置窗口继承详情页保存的亮暗主题，并响应跨窗口主题变化。
- 放大更新日志和 Radar 研判字体，将“发重置卡”与“硬重置”改为上下两行布局。

### 0.11.0

- 采用 View Transitions API 复刻 Element Plus 风格的主题切换：以主题按钮为圆心，通过新旧页面快照的圆形裁剪完成扩散与收缩。
- 版本信息区域改为常驻；没有可用更新时展示当前版本号、发布日期和本版更新日志。
- Radar 自动解析官网“重置雷达研判”中的“发重置卡”和“硬重置”两项动态标题，并保留旧信号字段作为兼容回退。
